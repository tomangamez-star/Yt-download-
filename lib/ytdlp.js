...
function createWritableCookiesCopy(sourceFile) {
  const suffix = crypto.randomBytes(8).toString('hex');
  const tempFile = path.join(os.tmpdir(), `tubegrab-cookies-${process.pid}-${suffix}.txt`);
  fs.copyFileSync(sourceFile, tempFile);
  fs.chmodSync(tempFile, 0o600);
  return tempFile;
}

function cookieArgs(useCookies = false, cookiesFileOverride = null) {
  if (!useCookies) return [];
  const cookiesFile = cookiesFileOverride || resolveCookiesFile();
  return cookiesFile ? ['--cookies', cookiesFile] : [];
}

function runYtDlp(args, { timeoutMs = DEFAULT_TIMEOUT_MS, onStdout, onStderr, useCookies = false, cookiesFileOverride = null } = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(YTDLP_BIN, ['--js-runtimes', 'node', ...cookieArgs(useCookies, cookiesFileOverride), ...args], {
...
async function downloadVideo({ url, quality, type, outputDir, maxFileMb, onProgress, onPhase }) {
  ...
  const runDownload = (useCookies, cookiesFileOverride = null) => runYtDlp(args, {
    timeoutMs: 0,
    useCookies,
    cookiesFileOverride,
  ...
  try {
    ...
    console.log('[yt-dlp] Public download hit a YouTube auth challenge; retrying with a writable copy of Render cookies.');

    // Render Secret Files are mounted read-only, but yt-dlp may update the
    // cookie jar while it runs. Copy the secret to /tmp so yt-dlp can safely
    // write to its own private working copy without touching the original.
    let writableCookiesFile = null;
    try {
      writableCookiesFile = createWritableCookiesCopy(cookiesFile);
      console.log(`[yt-dlp] Created temporary writable cookie jar at ${writableCookiesFile}`);
      ...
      await runDownload(true, writableCookiesFile);
    } finally {
      if (writableCookiesFile) {
        try {
          fs.rmSync(writableCookiesFile, { force: true });
          console.log('[yt-dlp] Removed temporary writable cookie jar.');
        } catch (cleanupError) {
          console.warn(`[yt-dlp] Could not remove temporary cookie jar: ${cleanupError.message}`);
        }
      }
    }
  }
