...
function cookieArgs() {
  ...
  if (!fs.existsSync(COOKIES_FILE)) {
    console.warn(`[yt-dlp] YTDLP_COOKIES_FILE does not exist: ${COOKIES_FILE}`);
    return [];
  }
  return ['--cookies', COOKIES_FILE];
}

function runYtDlp(args, { timeoutMs = DEFAULT_TIMEOUT_MS, onStdout, onStderr } = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(YTDLP_BIN, ['--js-runtimes', 'node', ...cookieArgs(), ...args], {
